...
type _ExclusifyUnion<T, K extends PropertyKey> = T extends unknown
  ? Prettify<T & Partial<Record<Exclude<K, keyof T>, never>>>
  : never;
...
type MergeKeyValue<AVal, BVal> = [BVal] extends [never]
...
export type HaveSameKeys<A, B> = keyof A extends keyof B

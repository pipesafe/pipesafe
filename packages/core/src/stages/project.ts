...
export type HasInclusionsNonId<P> = true extends {
  [K in keyof P]: K extends "_id"
    ? never
    : P[K] extends 1 | true
      ? true
      : never;
...
export type HasExclusionsNonId<P> = true extends {
  [K in keyof P]: K extends "_id"
    ? never
    : P[K] extends 0 | false
      ? true
  ...
  DeepMergeProjection<
    MergeNested<
      {},
      {
        ...
        // branded at dispatch.
        [
          K in keyof NonDottedKeys<Query> as K extends "_id"
            ? never
            : Query[K] extends 0 | false
              ? never
              : K
        ]: ResolveFieldValue<Schema, Query[K], K & string>;
...
type ResolveExclusionMode<Schema extends Document, Query> = Prettify<
  {
    ...
    [
      K in keyof Schema as K extends "_id"
        ? never
        : K extends keyof Query
          ? Query[K] extends 0 | false
            ? never
    ...
    [
      K in keyof Query as K extends "_id"
        ? never
        : Query[K] extends `$${string}`
          ? K
          : Query[K] extends Document

...
export type FieldPath<T> = T extends (infer U)[]
  ? FieldPath<U>
  : T extends object
    ? T extends NonExpandableTypes
      ...
      : {
          [K in Extract<keyof T, string>]:
            | K
            | (FieldPath<T[K]> extends infer P extends string
                ? Join<K, P>
                : never);
...
type RefsInferringTo<M, DesiredType> = {
  [K in keyof M]: M[K] extends PipeSafeError<string>
    ...
    : NonNullable<M[K]> extends DesiredType
      ? K
      : never;
}[keyof M];
...
export type FieldPathsThatInferToForLookup<
  ...
  DesiredType,
> =
  FieldReferencesThatInferToForLookup<Schema, DesiredType> extends never
    ? never
    : FieldReferencesThatInferToForLookup<
          Schema,
          DesiredType
        > extends `$${infer Path}`
      ? Path
...
export type FieldReferencesThatInferTo<
  Schema extends Document,
  DesiredType,
> = Schema extends unknown
...
export type InferNestedFieldReference<Schema extends Document, Obj> =
  Obj extends FieldReference<Schema>
    ? InferFieldReference<Schema, Obj>
    : // Nested `$$REMOVE` deletes the field: `never` here is load-bearing —
      ...
      : // Other `$$`-system variables ($$NOW, $$ROOT, ...) are valid MongoDB
        ...
        : // Unknown single-`$` refs: validation brands them at the call site, so
          ...
          ? never
          : Obj extends unknown[]
            ? InferNestedFieldReferenceArray<Schema, Obj>
            : Obj extends object
...
> = Arr extends []
  ...
  : Arr extends [infer First, ...infer Rest]
    ...
    : Arr extends (infer Item)[]
      ? InferNestedFieldReference<Schema, Item>[]
      : never;

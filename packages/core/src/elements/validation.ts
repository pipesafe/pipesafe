    ...
    : [OperatorKeyOf<V>] extends [keyof ExpressionSpec<Schema>]
      ? // $concat gets an element-wise walk (mirrors group's $min/$max): the
        ...
        : // Schema-FREE fast-accept (mirrors group's ValidateAccumulatorValue):
          ...
          [V] extends [ExpressionFor<{}, OperatorKeyOf<V>>]
          ? never
 ...
 */
type ValidateConcatValue<Schema extends Document, V> = V extends {
  readonly $concat: infer Arr;
}
...
type ValidateConcatElement<
  ...
  E,
> = E extends SystemVariable
  ? never
  : E extends `$$${string}`
    ? UnknownSystemVariableError<E>
    : E extends `$${string}`
      ? string extends keyof Schema
          ...
          ? V extends NonExpandableTypes
            ...
            : string extends keyof V
              ...
              : HasOperatorKey<V> extends true
                ? ValidateExpressionValue<Schema, V>
 ...
 */
type ValidateObjectValue<Schema extends Document, V> = true extends {

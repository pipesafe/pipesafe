  ...
  Schema extends (infer U)[]
    ? Path extends `${infer Index}.${infer Rest}`
      ? Index extends `${number}`
        ...
        : GetFieldType<U, Path> extends infer R // Non-numeric - field access on elements
          ? R extends never
            ? never
            : R[]
          : never
      : Path extends `${number}`
        ...
        : GetFieldType<U, Path> extends infer R // No dots - field access on elements
          ? R extends never
            ? never
            : R[]
          : never
...
export type FieldSelectorsThatInferTo<Schema extends Document, DesiredType> = {
  [K in FieldSelector<Schema>]: InferFieldSelector<
    Schema,
    K
  > extends DesiredType
    ? K
...
export type TopLevelField<T extends string> = T extends `${string}.${string}`
  ? never
  : T;

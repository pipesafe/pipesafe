...
[![npm version](https://img.shields.io/npm/v/tmql.svg)](https://www.npmjs.com/package/tmql)
...
tmql supports three flexible connection patterns, each offering different advantages and use cases. The **Singleton Pattern** is most similar to Mongoose's `mongoose.connect()` approach.

### Pattern 1: Singleton Pattern (Similar to Mongoose)

Connect once using `tmql.connect(uri)` and access databases/collections through the singleton instance.
...
Create `TMDatabase` or `TMCollection` instances directly with a client.
...
For DAG model composition and orchestration features (TMModel, TMProject), see the companion package [`tmql-orchestration`](https://www.npmjs.com/package/tmql-orchestration).

## Status

tmql is actively under development. Contributions, feedback, and suggestions are welcome!

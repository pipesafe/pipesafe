...
- Match operand helpers (`NumericOperand`, `SizeOperand`, etc.) switched from naked-conditional distribution to `[T] extends [...]` so a field typed as a union (e.g. `status: 'pending' | 'shipped' | 'delivered'`) hovers with the full union as the brand's `Ctx` rather than a single arbitrary union member.

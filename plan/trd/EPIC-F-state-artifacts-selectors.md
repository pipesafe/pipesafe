...
- **PR-review addendum (2026-07, see [../07-open-pr-review.md](../07-open-pr-review.md))**: the throw-on-function stance (spike F5) stands for raw functions in stage arrays (`custom()` payloads like `$where`). PR #101 — expected to land reworked — serializes `$function` bodies to strings at stage-add time inside `_chain`, so builder-produced `$function` stages reach hash time as **already-serialized strings** and never hit the throw path: hash the string as-is (deterministic within a build). But that text is transpiler-dependent — `Function.prototype.toString` returns _transpiled_ source (bun vs tsc vs swc vs a minifying app bundler all emit different text for the same TS source), so a `$function`-bearing model risks a spurious `state:modified` whenever the build toolchain changes. Mitigation to decide at implementation time: a normalization pass over serialized bodies (e.g. whitespace-stripping or parse + canonical re-print) or a documented "version-pin your toolchain" caveat for `$function`-bearing models.

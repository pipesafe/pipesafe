...
- **Update operators**: new work, but small and formulaic. `$set` reuses `set.ts`'s dotted-key machinery; `$inc/$mul/$min/$max` brand wrong-typed operands with `PipeSafeError` at the value position (TS2322 per the established convention); `$push/$addToSet/$pull` validate element types against the array's element type; `$unset` reuses `unset.ts` paths. (Corrected by the EPIC-B spike — see trd/EPIC-B-typed-crud.md F6: the originally prescribed key restriction to `FieldSelectorsThatInferTo<Schema, number>` silently drops optional numeric fields — `$inc: { age: 1 }` on `age?: number` would be falsely rejected — and yields unreadable errors; the verified design maps **all** `FieldSelector` keys and brands at the value position with `Exclude<T, undefined>` before the tuple check.)
...
Signature pattern: `<const F extends MatchQuery<Schema>>(filter: F & ValidateMatchKeys<Schema, F>, options?)` so the literal drives both validation and result narrowing. (Corrected by the EPIC-B spike — see trd/EPIC-B-typed-crud.md F2/F3: the plain generic constraint alone does **not** reject typo'd paths on array-bearing schemas — array-index template-literal selectors create pattern index signatures that defeat excess-property checking, and a typo next to a valid key always compiles, silently returning `never`; `Pipeline.match` has the identical hole today. The verified fix is intersecting a homomorphic `ValidateMatchKeys` mapped type that brands unknown keys and recurses through `$and`/`$or`/`$nor`.) Mongoose v9's safety affordances come near-free: an `orFail` option and `sanitizeFilter`-style runtime `$`-key wrapping on user input. Raw-driver methods stay available via an explicit `collection.raw()` escape hatch — same philosophy as `Pipeline.custom()`. Effort ~3–4 weeks; the compile-perf risk on CRUD typing did not materialize (EPIC-B spike measured ~400–500 instantiations per validated call site — an order of magnitude below one `$project` stage; benchmarks stay as a guard).
...
Effort: bridge + `$jsonSchema` deriver ~4 weeks, with a documented "unrepresentable ⇒ omitted from validator" rule. (Corrected by the EPIC-C spike — see trd/EPIC-C-runtime-schema-validation.md: Standard Schema exposes _only_ `validate()`, no introspection API and no standard-adjacent metadata, so a generic `deriveJsonSchema(schema: StandardSchemaV1)` is unimplementable. The deriver is **per-vendor adapters** dispatched on `~standard.vendor` — Zod v4's `z.toJSONSchema`, Valibot's `@valibot/to-json-schema`, post-processed into Mongo's draft-4 dialect. Validation works for any vendor; validator derivation only for adapted vendors, with an explicit `bsonSchema` override as the fallback.) The Standard Schema dependency is one tiny spec package.

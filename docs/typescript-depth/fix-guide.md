...
2. If still too deep, unroll 2–3 levels per step. See [§2.2](./limits.md#22-loop-unrolling-process-n-elements-per-step) and the `ExpandDottedKeyBatched` pattern.
...
The lint plugin (`tools/eslint-plugin-pipesafe-types`) flags the highest-leverage anti-patterns at authoring time:

- `prefer-tail-recursion` — head recursion in conditional types.
- `require-non-distributive-extends` — naked `T extends U` over union-typed generics.
- `no-prettify-as-fix` — `Prettify` wrapping a known-recursive alias (which doesn't help).
- `no-mutable-alias-in-brand` — `Mutable<T>` inside `PipeSafeError`.
- `flag-deep-mapped-over-keyof-in-recursive` — inline mapped types inside self-recursive aliases.

Per-rule rationale lives in `lint-rules/`. Run `bun run lint` to catch these before they hit `bun run build`.

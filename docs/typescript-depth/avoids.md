...
- **`[T] extends [U]`** for union-stable conditionals. Seen at `match.ts:32-52, 83-86`, `graphLookup.ts:11`.
- **Accumulator-style tail recursion.** Recursive call is the last thing the false branch returns, no wrapper.
- **Loop unrolling at 2–3 segments per step.** See `ExpandDottedKeyBatched` (`utils/core.ts:52-65`). Don't unroll past 3 without measuring.
- **Named intermediate aliases** for any computation reused twice. The type-alias cache is the cheapest win available.
- **`extends infer` to share a result across multiple inner branches** (not to chain steps). See `utils/core.ts:130, 179, 196, 233, 252, 266, 450, 487, 564`.
- **`NonExpandableTypes`** (`utils/core.ts:84`) as a stop-list whenever a recursive walk could otherwise drill into Date / BSON / Function. Add new opaque types here as the schema vocabulary grows.

...
export type $Error<M extends string, T = unknown> = M extends any 
  ? $Type<'error', { message: M; type: T }> 
  : never
...
export type $Type<T extends string, V, $O> = $O extends { bare: true }
  ? $Type.$<T, V>
...
export type Brand<B extends string, T = never> = 
  [T] extends [null | undefined | symbol | void]
    ? Branded<B, T>

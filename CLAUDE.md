...
- **literals.ts**: Literal value type constraints — `AnyLiteral`,
  ...
  ONE environment mechanism for system and user variables alike:
  ...
  ties the two) seed `SystemVariables<Schema>` — THE default of every
  `Vars` parameter. `$let`/`$map`/`$filter` binding arms EXTEND the same
  environment (BindLetVars/BindVariable), `$lookup.let` reseeds it over the
  foreign schema through Pipeline's `Env` generic (PipelineVars), and
  resolution is a single env lookup: `InferVariableReference` /
  `ValidateVariableReference` (dotted `"$$name.path"` included — forgiving
  for inference, branding for validation). `VariableReferences<Vars>`
  derives the finite acceptance vocabulary (exact names + dotted paths into
  document-typed entries) for top-level value unions, and
  `SystemVariablesThatInferTo` (the `$$` sibling of
  FieldReferencesThatInferTo) the typed operand sets.

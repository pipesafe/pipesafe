...
│   │   │   ├── function-helpers/ # $function runtime: body serialization, purity check, server globals
...
- **`$function` (elements/, not a stage)**: server-side JavaScript expression operator, usable in any expression position
  ...
  - **Runtime**: `function-helpers/serializeFunction.ts` serializes bodies at the `Pipeline._chain` choke point — acorn free-variable purity check (must be self-contained), arrow→`function(){}` wrapping (MongoDB requirement), `serverFn` refs bundled via the optional `@pipesafe/function-bundler` package
...
- **`$function` purity is enforced by lint + runtime, not the type system** — function bodies are opaque to types. Both layers run ONE shared analysis (`function-helpers/analyzeFunctionBody.ts`, strict-mode scoping): the `no-impure-function-body` ESLint rule executes it over ESLint's AST (editor/CI), the runtime executes it over acorn's parse of the body source at stage-add time (`function-helpers/serializeFunction.ts`) — so the two cannot drift. The lint rule adds ONE scope-aware check the runtime is structurally blind to (a closure over a user binding shadowing a server-global name; a `toString` source carries no lexical context). Bodies needing imports use `serverFn()` + the optional `@pipesafe/function-bundler` package (bundle cache revalidated by content hash of all bundled files).
